...
MODES = {
    ...
    "🐛 Fix It": {
        ...
        "followup_prompt": (
            ...
            "web search results above. IMPORTANT: only state facts, sources, or "
            "URLs that literally appear in the search results text above — never "
            "invent a URL, product page, or citation. If the search results don't "
            "actually contain a relevant answer, say so honestly instead of "
            "guessing or making something up. Keep it clear and encouraging, no "
            "unnecessary jargon."
        ),
    },
    "🍳 Cook It": {
        ...
        "followup_prompt": (
            ...
            "the web search results above rather than assuming. IMPORTANT: only "
            "state facts, sources, or URLs that literally appear in the search "
            "results text above — never invent a URL, product page, or citation. "
            "If the search results don't actually contain the real ingredient "
            "list, say so honestly (e.g. 'the web search didn't return the exact "
            "ingredients, so here's a reasonable recipe based on the visible "
            "items instead') rather than fabricating a source. Give a short "
            "ingredient list and clear numbered steps."
        ),
    },
    "🎯 Anything Else": {
        "desc": "Any photo → ask it whatever you want",
        "vision_prompt": (
            "You may be given one or more photos. Describe everything relevant "
            "in detail: objects, text, brands/products, numbers, context — "
            "anything that could help answer a follow-up question about it. If "
            "any brand or product name is visible, state it clearly on its own "
            "line starting with 'BRAND:'."
        ),
        "followup_label": "What do you want to know about this photo?",
        "search_query_template": "{vision_output}",
        "followup_prompt": (
            "Here is what was seen in the photo(s):\n{vision_output}\n\n"
            "Here is what a web search found (use this for accurate facts if "
            "relevant, but only if it's actually relevant to the question):\n"
            "{search_results}\n\n"
            "The user's question is:\n{user_input}\n\n"
            "Answer the user's question directly and clearly, using the photo "
            "context and web search results where relevant. IMPORTANT: only "
            "state facts, sources, or URLs that literally appear in the search "
            "results text above — never invent a URL, product page, or citation. "
        ...
        if st.button("✨ Get my answer", type="primary"):
            ...
            if "search_query_template" in MODES[mode]:
                with st.spinner("Searching the web for accurate info..."):
                    if user_input.strip():
                        # combine user's request with identified brand/context
                        # so the query stays targeted instead of going generic
                        lines = st.session_state.vision_output.splitlines()
                        brand_lines = [
                            l.replace("BRAND:", "").strip() for l in lines if "BRAND:" in l
                        ]
                        context = " ".join(brand_lines[:2]) if brand_lines else " ".join(lines[:2])
                        query = f"{context} {user_input.strip()}"[:390]
                    else:
